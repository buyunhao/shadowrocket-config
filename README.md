...
补充区块，合并手工自定义规则和 `rules/adblock-exceptions.list` 后重新生成
...
`Johnshall/Shadowrocket-ADBlock-Rules-Forever` 的 `sr_ad_only.conf` 规范化生成，并依次
补充 AWAvenue Ads Rule 中尚未被 Johnshall 覆盖的规则，以及 anti-AD 中尚未被前两层
覆盖的规则。生成过程会再合并本地手工规则并应用精确例外；最终规则由父配置统一应用
`REJECT` 策略。三份上游内容分别沿用其 CC BY-SA 4.0、GPL-3.0 和 MIT 许可。

两份本地输入文件的用途如下：

- `rules/adblock-custom.list`：脚本管理其中的 AWAvenue 与 anti-AD 补充区块；区块之外
  用于手工补充已经通过日志或实际测试确认、但三份上游尚未收录的广告规则。
- `rules/adblock-exceptions.list`：从“上游 + 自定义规则”的结果中排除确认误杀的规则。

每次同步都会刷新 `adblock-custom.list` 中各自标记之间的 AWAvenue QuantumultX 规则和
anti-AD Surge 规则。AWAvenue 先按 Johnshall 当次主规则的实际覆盖范围做语义去重；
anti-AD 再按 Johnshall 与已保留的 AWAvenue 规则去重。具体来源版本、文件哈希、许可和
导入数量记录在各自的自动生成区块中；区块外的手工规则和注释不会被改写。
...
1. 下载、规范化并校验 Johnshall、AWAvenue 和 anti-AD 三份上游规则。
2. 将 AWAvenue 与 Johnshall 语义去重，再将 anti-AD 与前两层语义去重，刷新
   `adblock-custom.list` 的两个受管区块。
